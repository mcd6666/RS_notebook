...
def _weighted_mean(values: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    if total <= 0:
        raise RuntimeError("iMAD weights collapsed to zero")
    return np.sum(values * weights[None, :], axis=1) / total


def _weighted_cov(x: np.ndarray, y: np.ndarray, weights: np.ndarray) -> np.ndarray:
    weights = weights.astype(np.float64)
    total = float(np.sum(weights))
    if total <= 0:
        raise RuntimeError("iMAD weights collapsed to zero")
...
def _imad_score(
    ...
    if n < max(100, bands * 20):
        raise RuntimeError(f"Only {n} valid iMAD candidate pixels found")

    # X is target/GF, Y is reference/S2. Columns are pixels.
    x_all = target[:, ys, xs].astype(np.float64)
    y_all = reference[:, ys, xs].astype(np.float64)
    ...
    for itr in range(max_iter):
        ...
        sxx = _weighted_cov(x, x, weights) + np.eye(bands) * reg
        syy = _weighted_cov(y, y, weights) + np.eye(bands) * reg
        sxy = _weighted_cov(x, y, weights)
        syx = sxy.T

        inv_sxx = np.linalg.pinv(sxx)
        inv_syy = np.linalg.pinv(syy)
        mat = inv_sxx @ sxy @ inv_syy @ syx
        eigvals, a = np.linalg.eig(mat)
        order = np.argsort(np.real(eigvals))[::-1]
        eigvals = np.clip(np.real(eigvals[order]), 0.0, 1.0)
        a = np.real(a[:, order])

        # Normalize canonical vectors to unit variance in their own image.
        for idx in range(bands):
            av = float(np.sqrt(max(a[:, idx].T @ sxx @ a[:, idx], 1e-12)))
            a[:, idx] /= av

        rho = np.sqrt(eigvals)
        b = inv_syy @ syx @ a
        for idx in range(bands):
            bv = float(np.sqrt(max(b[:, idx].T @ syy @ b[:, idx], 1e-12)))
            b[:, idx] /= bv
            if rho[idx] > 1e-8 and np.corrcoef(a[:, idx] @ x, b[:, idx] @ y)[0, 1] < 0:
                b[:, idx] *= -1.0
    ...
    if mad is None:
        raise RuntimeError("iMAD failed to produce MAD variates")

    mad_var = np.nanvar(mad, axis=1)
    mad_var = np.where(mad_var > 1e-12, mad_var, 1.0)
    chi2 = np.sum((mad * mad) / mad_var[:, None], axis=0)
    score = np.full(candidate_mask.shape, np.nan, dtype=np.float32)
    score[ys, xs] = chi2.astype(np.float32)
    return score
...
def select_pif_mask(
    ...
    if method == "imad":
        ...
        raise RuntimeError(f"Unsupported PIF method: {method}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=("imad", "score"),
        default="imad",
        help="PIF selection method. imad is more reliable; score is faster. Default: imad",
    )
    parser.add_argument(
        "--imad-iter",
        type=int,
